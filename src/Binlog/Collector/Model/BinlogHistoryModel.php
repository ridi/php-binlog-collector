...
class BinlogHistoryModel extends BinlogHistoryBaseModel
{
	...
	private function insertBinlogPosition(Collection $dtos): Collection
	{
		...
		if ($insert_binlog_infos->count() > 0) {
			...
			$this->db->sqlInsertBulk(
				'platform_universal_history_3_binlog',
				$keys,
	...
	private function insertRows(Collection $dtos): Collection
	{
		...
		if ($insert_rows->count() > 0) {
			...
			$this->db->sqlInsertBulk(
				'platform_universal_history_3_row',
				$keys,
				$insert_rows->all()
			);
		}
	...
	private function getRowDictsByBinlogIds(array $binlog_ids): array
	{
		...
		return $this->db->sqlDicts(
			'SELECT id, binlog_id, idx, table_name, pk_ids FROM platform_universal_history_3_row WHERE ?',
			sqlWhere($where)
		);
	}
	...
	private function insertColumns(Collection $dtos)
	{
		...
		if ($column_rows->count() > 0) {
			...
			$this->db->sqlInsertBulk(
				'platform_universal_history_3_column',
				$keys,
				$column_rows->all()
			);
		}
	...
	private function getColumnDictsByRowIds(array $row_ids): array
	{
		...
		return $this->db->sqlDicts(
			'SELECT id, row_id, `column` FROM platform_universal_history_3_column WHERE ?',
			sqlWhere($where)
		);
	}
	...
	public function getRecentEmptyGtidBinlogId()
	{
		...
		return $this->db->sqlData(
			'SELECT id FROM platform_universal_history_3_binlog WHERE ? ORDER BY id DESC ?',
			sqlWhere($where),
			sqlLimit(1)
		);
	}
	...
	public function getEmptyGtidBinlogDictsByLesserEqualId(int $id, int $limit): array
	{
		...
		return $this->db->sqlDicts(
			'SELECT * FROM platform_universal_history_3_binlog WHERE ? ORDER BY id DESC ?',
			sqlWhere($where),
			sqlLimit($limit)
		);
	}
	...
	public function updateBinlogGtid(int $id, string $gtid)
	{
		...
		return $this->db->sqlUpdate('platform_universal_history_3_binlog', $update, $where);
	}
		...
		return $this->db->sqlData(
			"SELECT COUNT(*)
				...
				INNER JOIN platform_universal_history_3_row AS u_row
					ON u_row.binlog_id = u_binlog.id
				WHERE ?
			",
			sqlWhere($where)
		);
	...
	public function getBinlogColumnCount(array $where): int
	{
		return $this->db->sqlData(
			"SELECT COUNT(*)
				...
				INNER JOIN platform_universal_history_3_row AS u_row
					ON u_row.binlog_id = u_binlog.id
				INNER JOIN platform_universal_history_3_column AS u_column 
					ON u_column.row_id = u_row.id
				WHERE ?
			",
			sqlWhere($where)
		);
